...
import { Search, Trash2, Hash, Check, Bookmark } from 'lucide-react';
...
interface LibraryViewerProps {
  ...
  onSelectItem: (item: RepositoryItem | Template) => void;
}
      ...
      <div className="p-4 border-b border-border-visible space-y-4 bg-bg-primary/80 backdrop-blur-md">
        ...
        <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
          <button
            ...
            className={`px-3 py-1 text-[10px] font-medium uppercase tracking-wider border transition-all whitespace-nowrap ${
          ...
          {availableTags.map(tag => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1 text-[10px] font-medium uppercase tracking-wider border transition-all whitespace-nowrap flex items-center gap-2 ${
              ...
              >
                <div className="flex justify-between items-start gap-2 mb-2">
                  <div className="flex items-center gap-2 overflow-hidden">
                    {isSystem ? <Bookmark size={12} className="text-accent-primary/60 shrink-0" /> : <Hash size={12} className="text-text-tertiary opacity-40 shrink-0" />}
                    <h4 className="text-sm font-normal text-text-secondary group-hover:text-accent-primary transition-colors truncate">
                      {(item as any).title}
                    </h4>
                  </div>
                  {!isSystem && (
                ...
                <p className="text-[11px] text-text-tertiary line-clamp-1 leading-relaxed mb-2 italic font-normal opacity-70">
                  {item.description || 'Sem descrição.'}
                </p>
                <div className="flex flex-wrap gap-1">
                  {item.tags?.slice(0, 3).map(tag => (
                    <span key={tag} className="px-1 py-0.5 bg-bg-primary/50 text-text-tertiary border border-border-visible text-[9px] font-medium uppercase tracking-wide">
                      {tag}
                    </span>
                  ))}
                </div>
